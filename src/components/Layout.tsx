...
export default function Layout() {
  ...
  const tab = ({ isActive }: { isActive: boolean }) =>
    `rounded-lg px-3 py-1.5 text-sm font-medium ${isActive ? 'bg-brand-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`
      ...
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center gap-4 px-4 py-3">
          <div className="mr-2 flex items-center gap-3">
            <img src={logo} alt="First Medical Company" className="h-9 w-auto" />
            <div className="hidden border-l border-slate-200 pl-3 text-[11px] leading-tight text-slate-400 sm:block">
